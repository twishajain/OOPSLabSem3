...
class Menu
{
	...
	void writeIntoFile()
	{
		...
		try
		{
			FileOutputStream f = new FileOutputStream("original.txt");  //write into original.txt
	...
	void copyIntoFile()
	{
		try
		{
			...
			FileOutputStream writer = new FileOutputStream("duplicate.txt");  //copy into dupliacte.txt
			int i = 0;
	...
	void readFromFile()
	{
		...
		try
		{
			...
			int i = 0;
			while((i = f.read()) != -1)
				System.out.print((char) i);   //traversing file to display
			System.out.println("");
